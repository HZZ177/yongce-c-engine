...
class DspClient:
    ...
    def connect(self, device_type='1'):
        """
        模拟设备上线
        连接车场,并开启心跳线程和接受线程
        :return:
    ...
    def send_etc(self, i_serial, i_is_etc, i_etc_no, i_in_out_type, i_cap_time):
        ...
        try:
            ...
            # 取卡取票类型 0:取卡, 1:刷卡, 2:取票；5:验证码；6:远距离卡号(华为)
...
if __name__ == '__main__':
    # def send_img(self, i_serial, i_plate_no, i_car_style, i_is_etc, i_etc_no, i_recog_enable, i_color,
    #              i_data_type, i_open_type, i_cap_time=datetime.datetime.now()):
    #     """
    #     发送图片
    #     :param dsp_ip:设备IP
    #     :param i_serial: 序列号
    ...
    import time
    import random
    from testCases.Public import car, order
    # car.del_OnPark("川AHJH888", "592011611")
    # order.del_order("90127049743683904")
    # order.del_Bill("川AHJH888")
    # order.update_bus_order("90127049743683904")
    # dsp_obj_in = DspClient("192.168.0.166", 5001, "192.168.0.144", client_port=0)
    # dsp_obj_in = DspClient("192.168.0.198", 5001, "192.168.20.210", client_port=0)
    dsp_obj_in = DspClient("192.168.0.202", 5001, "192.168.0.144", client_port=0)
    # dsp_obj_in = DspClient("192.168.0.114", 5001, "192.168.0.148", client_port=0)
    # dsp_obj_out = DspClient("192.168.0.166", 5001, "192.168.0.145", client_port=0)
    # dsp_obj_out = DspClient("192.168.0.198", 5001, "192.168.20.211", client_port=0)
    dsp_obj_out = DspClient("192.168.0.202", 5001, "192.168.0.145", client_port=0)
    # dsp_obj_out = DspClient("192.168.0.114", 5001, "192.168.0.147", client_port=0)
    dsp_obj_in.connect()
    dsp_obj_out.connect()
    # i_serial = random.randint(0, 999999999)
    # dsp_obj_in.send_img("", "川AA8888", 0, 0, "", 1000, 3, 0, 0, i_cap_time=datetime.datetime.now())
    # dsp_obj_in.send_img("", "川AHJH999", 0, 0, "", 1000, 3, 0, 0, i_cap_time=datetime.datetime.now())
    while True:
        x = input("是否出车Y/y：")
        if x == "y" or x == "Y":
            break
    # dsp_obj_out.send_img("", "川AHJH999", 0, 0, "", 1000, 3, 0, 0, i_cap_time=datetime.datetime.now())
    # time.sleep(60)
    time.sleep(10)
    dsp_obj_in.close()
    dsp_obj_out.close()
